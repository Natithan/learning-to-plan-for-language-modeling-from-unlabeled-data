...
class Torchish:
  ...
  def __lt__(self, other):
    if isinstance(other, torch.Tensor):
        other = Torchish(other)
    return jnp.greater(other.value, self.value)
    
  def __le__(self, other):
    if isinstance(other, torch.Tensor):
        other = Torchish(other)
    other_value_on_same_device = jax.device_put(other.value, self.value.device_buffer.device())
    return jnp.less_equal(self.value, other_value_on_same_device)
  ...
  def squeeze(self, dim=None):
    if dim is None:
      ...
      return Torchish(jnp.squeeze(self.value, axis=dim))
      
  def unsqueeze(self, dim=None):
    return Torchish(jnp.expand_dims(self.value,axis=dim))
...
def embedding(input, weight, padding_idx=None, max_norm=None, norm_type=2.0, scale_grad_by_freq=False, sparse=False):
    assert padding_idx is None, "TODO: implement padding_idx"
    assert max_norm is None, "TODO: implement max_norm"
    assert norm_type == 2.0, "TODO: implement norm_type"
    assert not scale_grad_by_freq, "TODO: implement scale_grad_by_freq"
    assert not sparse, "TODO: implement sparse"

    return weight[coerce(input)]


@implements(torch.nn.functional.leaky_relu)
def leaky_relu(input, negative_slope=0.01, inplace=False):
    assert not inplace, "TODO: implement inplace=True"
    return jax.nn.leaky_relu(coerce(input), negative_slope)
...
def t2j_function(f):
  def f1(*args):
    ...
    if isinstance(result, tuple):
      return tuple(x.value for x in result)
    else:
        return result.value
  return f1
...
def t2j(thing):
  if isinstance(thing, torch.Tensor):
    ...
    for i in range(len(thing)):
        elements.append(t2j(thing[i]))
    return tuple(elements)
  else:
    print(thing, "not implemented!")
